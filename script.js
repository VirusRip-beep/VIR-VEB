...
async function claimBonus() {  
    ...
    if(data.success) {
        balance = data.balance;
        document.getElementById('user-balance').innerText = balance;
        showNotice('🎁 УСПЕХ!', data.message || '+1000 VIR на баланс', true);  
        fetchUserData();
    } else {
...
async function activatePromo() {
    ...
    if(!code) return showNotice('Ошибка', 'Введите промокод!', false);

    let res = await fetch('/api/promocode', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
    ...
    if(data.success) {
        balance = data.balance;
        document.getElementById('user-balance').innerText = balance;
        showNotice('🎟 УСПЕХ!', data.message, true);
        promoInput.value = '';
        fetchUserData();
    } else {
        showNotice('❌ Ошибка', data.message, false);
